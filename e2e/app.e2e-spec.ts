...
describe('Starting tests for block-certs', function() {
  ...
  it('website title should be block-certs', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('block-certs');
    })
  });

  it('network-name should be blockcerts@0.0.2',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('blockcerts@0.0.2.bna');
    });
  });

  it('navbar-brand should be block-certs',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('block-certs');
    });
  });
